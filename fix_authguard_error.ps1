# Script to fix AuthGuard error and push to Git
# Usage: .\fix_authguard_error.ps1

Write-Host "=== Fixing AuthGuard Error ===" -ForegroundColor Cyan
Write-Host ""
...
git commit -m "Fix: Move publicPaths outside useEffect in AuthGuard"
if ($LASTEXITCODE -eq 0) {
...
if ($LASTEXITCODE -eq 0) {
    ...
    Write-Host ""
    Write-Host "✅ AuthGuard error fixed and pushed to GitHub!" -ForegroundColor Green
    Write-Host ""
