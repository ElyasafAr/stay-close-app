# Script to fix auth.ts import and push to Git
# Usage: .\fix_auth_import.ps1

Write-Host "=== Fixing auth.ts Import ===" -ForegroundColor Cyan
Write-Host ""
...
git commit -m "Fix: Add missing getData import in auth.ts"
if ($LASTEXITCODE -eq 0) {
...
if ($LASTEXITCODE -eq 0) {
    ...
    Write-Host ""
    Write-Host "✅ Import fixed and pushed to GitHub!" -ForegroundColor Green
    Write-Host ""
